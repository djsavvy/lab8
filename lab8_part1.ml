...
module MakeInterval (Endpoint : ORDERED_TYPE) = 
  struct
    ...
    let contains (intvl : interval) (x : Endpoint.t) : bool =
      match intvl with 
      | Empty -> false 
      | Interval (l, h) -> (compare l x <= 0) && (compare x h <= 0) 
    ...
    let intersect (intvl1 : interval) (intvl2 : interval) : interval =
      ...
      | Interval (l1, h1), Interval (l2, h2) -> 
          create (if compare l1 l2 <= 0 then l2 else l1) (if compare h1 h2 <= 0 then h1 else l2)
...
let intvl1 = IntInterval.create ~-10 10 ;;
let intvl2 = IntInterval.create 0 20 ;;
